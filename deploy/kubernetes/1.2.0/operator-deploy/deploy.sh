...
#usage()
#{
#    ${ECHO}  "deploy.sh [-s|--silent] [--operator-ns=] [--cte-ns=] [--remove] "
#    ${ECHO}  ""
#    ${ECHO}  "-h | --help       Displays this help"
#    ${ECHO}  "--tag=            Displays this help"
#    ${ECHO}  "--remove          Delete CipherTrust Transparent Encryption for Kubernetes Operator and CSI driver"
#    ${ECHO}  "--operator-ns=    Input the namespace in which to deploy the Operator"
#    ${ECHO}  "--cte-ns=      Input the namespace in which to deploy CipherTrust Transparent Encryption for Kubernetes CSI driver"
#    ${ECHO}  "-s | --silent     Deploys the CipherTrust Transparent Encryption for Kubernetes operator anda CipherTrust"
#    ${ECHO}  "                  Transparent Encryption for Kubernetes CSI driver silently in kube-system namespace unless"
#    ${ECHO}  "                  --operator-ns and/or --cte-ns options are supplied"
#    ${ECHO}  ""
#    ${ECHO}  "If -s|--silent is not specified, the script will prompt for entering namespace."
#    ${ECHO}  "If the namespace supplied does not exist, the script will prompt to create the same,"
#    ${ECHO}  "However, if the -s|--silent option is specified, the script fails if namespace"
#    ${ECHO}  "supplied does not exist. The silent option does not prompt for creating the namespace."
#}
...
deploy_cte_csi()
{
    ...
    ${OC_KUBECTL_CMD} apply -f ${DEPLOY_SCRIPT_PATH}/ctek8soperator-operatorgroup.yaml -n ${OPR_NS}
    if [ $? -ne 0 ]; then
        ${ECHO} "Error creating OperatorGroup: ctek8soperator-og"
        exit 1
    fi

    ${OC_KUBECTL_CMD} apply -f ${DEPLOY_SCRIPT_PATH}/ctek8soperator-subscription.yaml -n ${OPR_NS}
    if [ $? -ne 0 ]; then
    ...
    # This takes a few seconds. Wait for 15 seconds.
    sleep 15
	...
	if [ "x${OPR_STATUS}" = "xRunning" ] && [ $loop_ctr -eq 15 ]; then
		${ECHO} "."
		break
	else
            ${ECHO} -n "."
...
cleanup_deployment()
{
    ...
    ${OC_KUBECTL_CMD} delete sa cte-k8s-operator-cte-csi-controller cte-k8s-operator-cte-csi-node -n ${OPR_NS} 2>&1 >/dev/null
